...
pub fn init_stdout(level: LevelFilter) -> bool {
    builder()
        .appender_filter(level, |msg: &str| println!("{}", msg))
        .init()
}
...
pub fn init_stderr(level: LevelFilter) -> bool {
    builder()
        .appender_filter(level, |msg: &str| eprintln!("{}", msg))
        .init()
}
...
impl Builder {
    ...
    #[must_use]
    pub fn appender_range(
        mut self,
        from: Level,
        to: Level,
        appender: impl IntoAppender,
    ) -> Self {
...
impl<T> IntoAppender for T where T: Appender + 'static {
    fn into_appender(self) -> Arc<dyn Appender> {
        Arc::new(self)
    }
}
...
impl Log for TrivialLog {
    fn enabled(&self, metadata: &Metadata) -> bool {
        if let Some(guard) = self.guard() {
            return guard
                .as_ref()
                .map(|inner| inner.is_enabled(metadata.level()))
                .unwrap_or(false);
        }
        false
    }
